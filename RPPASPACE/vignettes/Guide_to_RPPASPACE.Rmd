...
2. If not already installed, install a version of R greater than 3.5 (and optionally RStudio).  
3. Run R or RStudio.  
4. Install all the dependent packages used by RPPASPACE from CRAN using the command: install.packages(c("methods", "doParallel", "foreach", "parallel", "iterators", "utils", "grDevices", "graphics", "stats", "bmp", "jpeg", "tiff", "png", "imager", "cobs", "splines", "nmle", "robustbase", "mgcv", "SparseM", "quantreg", "timeDate", "boot", "knitr", "rmarkdown") If these are not available for the version of R you are using, you may need to install them from source, possibly one at a time.   
5. Install the RPPASPACE package.
...
* Removed dependence of the third-party image processing software product ImageMagick being installed on the target machine.
...
* Improved code base - removed unused portions of code in package including 8 complete source files and updated some deprecated methods in R.
...
***
As described in the RPPASPACE package description file, RPPASPACE requires an R version greater than or equal to 3.5 and utilizes the following packages:  
